...
- **Major components**:
  - `engine/` — core modules (UI, state, i18n, book manager, reader).
    ...
    - `engine/BookLoader.js` — loads `books/index.json` and per-book manifests (`books/<id>/book.json`) and localized book metadata (`books/<id>/language/*.json`).
...
- **What not to change without checking**:
  ...
  - The JSON structure under `books/` (manifest keys and language index shape) — changing keys requires updates across `BookLoader.js`, `uiManager.js`, and templates.
